...
class ShipmentReceiveProcessIntegrationTest < ActionDispatch::IntegrationTest
  ...
  def test_receive_shipment_SKU
    ...
    assert_equal asn_detail.purchase_order_nbr, case_header.purchase_order_nbr, "Purcahse order pulled from asn detail"
  ...
  def test_create_serial_number_total_quantity_entered
    ...
    serial = []
    SerialNumber.where(case_id: case_id).each {|serial_number| serial << serial_number.serial_nbr}
    assert_equal ['1', '2', '3', '4'], serial, 'Check if the serial number'
