...
fixtures :asn_details
  ...
  test "with valid data" do
    asn = Shipment::ShipmentMaintenanceHeader::HeaderMaintenance.new
    app_parameters = {client: 'WM', warehouse: 'WH1', channel: 'RTL', building: 'DIST1'}
    response = asn.send(:valid_app_parameters?, app_parameters)
  ...
  test "validate data with incorrect asn type" do
    ...
    assert_equal(false, asn.send(:valid_data?, fields_to_update), "Incorrect asn type")
  end

  test "validate data with incorrect multiple fields" do
    asn =  Shipment::ShipmentMaintenanceHeader::HeaderMaintenance.new
    fields_to_update = { asn_type: 'invalid',  purchase_order_nbr: ' '}
    assert_equal(false, asn.send(:valid_data?, fields_to_update), "validate data with incorrect multiple fields")
   ...
   test "validate data with correct single field" do
     ...
     assert_equal(true, asn.send(:valid_data?, fields_to_update), "correct asn type ")
  end
  ...
  test "Valid hot item po" do
    ...
    response = asn.send(:valid_hot_item_po?, fields_to_update)
  ...
  test "Valid purchase order nbr blank or nill " do
    ...
    response = asn.send(:valid_purchase_order_nbr?, fields_to_update)
  ...
  test "Valid vendor nbr blank or nill " do
    ...
    response = asn.send(:valid_vendor_nbr?, fields_to_update)
